...
BOT_KEY: Final = '7335556245:AAFewTn9GwGlGq3TiKeI0aRw528jMH5dxGw'
#Initiate bot
...
def input(message):
    ...
    bot.send_message(message.chat.id, "Do you want to search for a one-way flight or a round-trip?", reply_markup=markup)
...
def handle_inputs(message):
    ...
    if state == STATE_WAITING_FOR_TRIP_DAYS:
        try:
            ...
            bot.send_message(chat_id, "Please enter a departure city in the following format 'city-country' (e.g. prague-czechia, london-united-kingdom, paris-france).")
        except ValueError:
            bot.send_message(chat_id, "You have entered wrong format. Please, enter an integer.")
    ...
    if state == STATE_WAITING_FOR_DEPARTURE:
        try:
            ...
            bot.send_message(chat_id, "Great! Now enter a destination city in the same format 'city-country' (e.g. prague-czechia, london-united-kingdom, paris-france).") 
        except ValueError:
            bot.send_message(chat_id, "You have entered wrong format. Please, try again.")
        ...
        try:
            ...
            bot.send_message(chat_id, "Great! Now enter a desired departure date in the format YYYY-MM-DD (e.g. 2024-08-03) or enter 'Anytime' for flexible search.")
        except ValueError:
            bot.send_message(chat_id, "You have entered wrong format or invalid date. Please, try again.")
        ...
        try:
            ...
            if user_data["Date"] != "anytime":
                markup = types.InlineKeyboardMarkup()
                markup.add(types.InlineKeyboardButton("Precise", callback_data="precise"))
                markup.add(types.InlineKeyboardButton("+-1 day", callback_data="one_day"))
                markup.add(types.InlineKeyboardButton("+-3 days", callback_data="three_days"))
                markup.add(types.InlineKeyboardButton("+-5 days", callback_data="five_days"))
                bot.send_message(chat_id,"Great! Do you want a precise date or it could be flexible?", reply_markup=markup)
            else:
                markup = types.InlineKeyboardMarkup()
                ...
                markup.add(types.InlineKeyboardButton("Up to 2 stops", callback_data="two_stops"))
                bot.send_message(chat_id,"Great! Do you want to search for a direct fligth or not?", reply_markup=markup)
        except ValueError:
            bot.send_message(chat_id, "You have entered wrong format. Please, try again.")
...
def callback_flexible_dates(callback):
    if callback.data in ['one-way', 'round-trip']:
        if callback.data == 'one-way':
            ...
            bot.send_message(callback.message.chat.id, "Please enter a departure city in the following format 'city-country' (e.g. prague-czechia, london-united-kingdom, paris-france).")
        else:
            user_data['Type'] = 'round-trip'
            bot.send_message(callback.message.chat.id, "How many days approximately do you want to stay?")
    ...
    if callback.data in ['precise', 'one_day', 'three_days', 'five_days']:
        ...
        markup.add(types.InlineKeyboardButton("Up to 1 stop", callback_data="one_stop"))
        markup.add(types.InlineKeyboardButton("Up to 2 stops", callback_data="two_stops"))
        bot.send_message(callback.message.chat.id,"Great! Do you want to search for a direct fligth or not?", reply_markup=markup)
        ...
        print(user_data)
        bot.send_message(callback.message.chat.id,"Great! Let's start searching. The process may take some time. \nPlease wait......")
...
#                 else:
#                     sent_message = bot.send_message(callback.message.chat.id, "0% FINISHED").message_id
#                     days = int(user_data["Trip_days"])
#                     first_flight_date = pd.DataFrame(first_flight_data)["date"]
#                     transformed_dates = first_flight_date.apply(lambda x: transform_dates(x, days))
#                     return_user_data = user_data.copy()
#                     list_return_flights = []
#                     percent_finished = 0
...
#                         sent_message = bot.send_message(callback.message.chat.id, f"{percent_finished}% FINISHED").message_id
...
#                     total_price = int(first["price"].replace(" Kč", "").replace(",", "")) + int(second["price"].replace(" Kč", "").replace(",", ""))
...
#         except Exception:
#             bot.send_message(callback.message.chat.id, "An error occured. Please try to input something different. Type /search to start over.")

# bot.send_message(callback.message.chat.id, "If you want to set an automatic repetition of this search, please enter  ")
